 ...
 */
 ...
 main(int argc, char *argv[])
 {
	 ...
	 int dst_port = 0;
	 char *dst_addr = NULL;
	 u_int32_t sequence_number = 0 /* default ISN is 0 */;
	 ...
	 while ((ch = getopt(argc, argv, "vhP:D:d:i:o:u:p:x:c:s:t:T:qf:")) != -1) {
		 switch (ch) {
		 case 'v':
			 verbose = 1;
			 break;
	 ...
	 /* options for CCEAP */
		 ...
		 case 't':
            ...
            if (!fp) {
                fprintf(stderr, "Could not open timing file: %s\n", timing_file);
                exit(ERR_EXIT);
            }
            ...
            if (fgets(line, sizeof(line), fp)) {
                ...
                for (ptr_line = line; ; ptr_line = NULL) {
                    int digit;
                    
                    token = strtok_r(ptr_line, ",", &saveptr);
                    if (token == NULL)
                        break;
                    
                    digit = token[0];
                    if (isdigit(digit) == 0) {
                        fprintf(stderr, "in timing file: '%c' is not a digit\n",
                            token[0]);
                        fclose(fp);
                        exit(ERR_EXIT);
                    }
                    ...
                    IAT_array_elements++;
                    if (IAT_array_elements > MAX_NUM_PREDEF_IATS) {
                        fprintf(stderr, "Too many IAT values in timing file\n");
                        fclose(fp);
                        exit(ERR_EXIT);
                    }
                }
            ...
            if (IAT_array_elements > 0) {
                ...
                fprintf(stderr, "No valid IAT values found in timing file\n");
                exit(ERR_EXIT);
            }
            break;
		 case 'T':
