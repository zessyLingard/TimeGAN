...
class discriminator(nn.Module):
    def __init__(self, input_features, hidden_dim, epochs, batch_size):
        ...
        self.optimizer = torch.optim.Adam(
            chain(self.rnn.parameters(), self.model.parameters())
        )
    ...
    def fit(self, x, x_hat):
        x_train, x_test, x_hat_train, x_hat_test = train_test_split(
            x, x_hat, test_size=0.2
        )
        x_train, x_hat_train = tensor(
            x_train, dtype=torch.float32, device=device
        ), tensor(x_hat_train, dtype=torch.float32, device=device)
        dataset_train = TensorDataset(x_train, x_hat_train)

        x_test, x_hat_test = tensor(
            x_test, dtype=torch.float32, device=device, requires_grad=False
        ), tensor(x_hat_test, dtype=torch.float32, device=device, requires_grad=False)
        ...
        with torch.no_grad():
            ...
            y_pred_final = np.squeeze(
                np.concatenate(
                    (
                        y_pred_real.cpu().detach().numpy(),
                        y_pred_fake.cpu().detach().numpy(),
                    ),
                    axis=0,
                )
            )
...
class predictor(nn.Module):
    def __init__(self, dim, hidden_dim, epochs, batch_size):
        ...
        self.batch_size = batch_size
        self.rnn = nn.GRU(
            input_size=(dim - 1) if dim > 1 else 1,
            hidden_size=hidden_dim,
            num_layers=self.num_layers,
            batch_first=True,
        )
        ...
        self.optimizer = torch.optim.Adam(
            chain(self.rnn.parameters(), self.model.parameters()), 1e-3
        )
    ...
    def fit(self, data_train, data_test):
        if self.dim == 1:
            x_train, y_train = tensor(
                data_train[:, :-1, :], dtype=torch.float32, device=device
            ), tensor(data_train[:, 1:, :], dtype=torch.float32, device=device)
        elif self.dim > 1:
            x_train = data_train[:, :-1, : (self.dim - 1)]
            y_train = np.reshape(
                data_train[:, 1:, (self.dim - 1)],
                (data_train.shape[0], data_train.shape[1] - 1, 1),
            )
            x_train, y_train = tensor(
                x_train, dtype=torch.float32, device=device
            ), tensor(y_train, dtype=torch.float32, device=device)
        ...
        if self.dim == 1:
            x_test = tensor(
                data_test[
                    torch.randperm(len(data_test), device=device).cpu().detach().numpy()
                ][:-1, :],
                dtype=torch.float32,
                device=device,
                requires_grad=False,
            ...
            y_test = np.reshape(
                data_test[:, 1:, (self.dim - 1)],
                (data_test.shape[0], data_test.shape[1] - 1, 1),
            )
            x_test, y_test = tensor(
                x_test, dtype=torch.float32, device=device, requires_grad=False
            ), tensor(y_test, dtype=torch.float32, device=device, requires_grad=False)
        ...
        with torch.no_grad():
            ...
            for i in range(len(pred_test)):
                MAE += mean_absolute_error(
                    y_test[i, :, :].cpu().detach().numpy(),
                    pred_test[i, :, :].cpu().detach().numpy(),
                )
                break
...
def visualization(ori_data, generated_data, analysis):
    ...
    prep_data_hat = np.reshape(
        np.mean(generated_data, axis=2), (anal_sample_no, seq_len)
    )
    ...
    if analysis == "pca":
        ...
        _plot_results(
            pca_results, pca_hat_results, colors, "PCA plot", "x-pca", "y-pca"
        )
    elif analysis == "tsne":
...
def _plot_results(data1, data2, colors, title, xlabel, ylabel):
    ...
    plt.scatter(
        data1[:, 0], data1[:, 1], c=colors[: len(data1)], alpha=0.2, label="Original"
    )
    plt.scatter(
        data2[:, 0], data2[:, 1], c=colors[len(data1) :], alpha=0.2, label="Synthetic"
    )
    plt.legend(
        loc="lower center", bbox_to_anchor=(0.5, 1), ncol=2, frameon=False, fontsize=15
    )
    ...
    plt.gca().xaxis.set_major_formatter(
        mticker.FuncFormatter(lambda x, _: "{:.2f}".format(x))
    )
    plt.gca().yaxis.set_major_formatter(
        mticker.FuncFormatter(lambda y, _: "{:.2f}".format(y))
    )
    plt.show()
