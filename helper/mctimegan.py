...
# =============================================================================
# ADDED: Conditioning Loss for Per-Timestep Enforcement
# =============================================================================
def conditioning_loss(x_hat, original_conds, threshold=0.5):
    """
    Enforce that generated values match the condition intent at each timestep.
    
    Args:
        x_hat: Generated output (batch, seq, 1) - values in [0, 1] after sigmoid
        original_conds: Original conditions before ConditioningNetwork (batch, seq, 1)
        threshold: Threshold to separate high/low conditions
    
    Returns:
        Loss that penalizes if high conditions don't produce higher outputs
    """
    # Flatten for easier computation
    x_flat = x_hat.view(-1)
    c_flat = original_conds.view(-1)
    
    # Masks for high (Bit 1) and low (Bit 0) conditions
    high_mask = c_flat > threshold
    low_mask = c_flat <= threshold
    
    # Compute mean output for each condition type
    if high_mask.any() and low_mask.any():
        mean_high = x_flat[high_mask].mean()
        mean_low = x_flat[low_mask].mean()
        
        # Penalize if mean_low >= mean_high (we want mean_high > mean_low)
        # margin ensures clear separation
        margin = 0.15
        loss = torch.relu(mean_low - mean_high + margin)
    else:
...
class ConditioningNetwork(nn.Module):
    ...
    def __init__(self, input_size, condition_size):
        super().__init__()
        # FIXED: Larger hidden layer to preserve per-timestep information
        hidden_size = max(16, condition_size * 2)
        self.condition = nn.Sequential(
            nn.Linear(input_size, hidden_size),
            nn.Tanh(),
            nn.Linear(hidden_size, condition_size),
            nn.Tanh()  # ADDED: Bound outputs to [-1, 1]
        )
...
class Supervisor(nn.Module):
    ...
    def __init__(self, module_name, input_features, hidden_dim, num_layers):
        ...
        self.rnn = rnn_class(
            input_size=input_features,
            hidden_size=hidden_dim,
            num_layers=max(1, num_layers - 1),  # FIXED: Ensure at least 1 layer
            batch_first=True,
        )
        self.model = nn.Sequential(nn.Linear(hidden_dim, hidden_dim), nn.Sigmoid())
...
def discriminator_loss(y_real, y_fake, y_fake_e):
    ...
    valid = torch.ones_like(
        y_real, dtype=torch.float32, device=device, requires_grad=False
    )
    fake = torch.zeros_like(
        y_fake, dtype=torch.float32, device=device, requires_grad=False
    )
...
class MCTimeGAN(nn.Module):
    ...
    def __init__(
        ...
        # FIXED: Increased cond_size for better per-timestep conditioning
        # Original was 1, which loses per-timestep distinction
        ...
        if input_conditions is not None:
            ...
            self.embedder = Embedder(
                module_name, input_features + self.cond_size, hidden_dim, num_layers
            )
            self.recovery = Recovery(
                module_name, input_features, hidden_dim + self.cond_size, num_layers
            )
            self.generator = Generator(
                module_name, input_features + self.cond_size, hidden_dim, num_layers
            )
            self.supervisor = Supervisor(
                module_name, hidden_dim + self.cond_size, hidden_dim, num_layers
            )
            self.discriminator = Discriminator(
                module_name, hidden_dim + self.cond_size, num_layers
            )
            # Define optimizers for different network components
            self.optimizer_e = torch.optim.Adam(
                chain(
                    self.condnet.parameters(),
            ...
            self.embedder = Embedder(
                module_name, input_features, hidden_dim, num_layers
            )
            self.recovery = Recovery(
                module_name, input_features, hidden_dim, num_layers
            )
            ...
            self.supervisor = Supervisor(
                module_name, hidden_dim, hidden_dim, num_layers
            )
            self.discriminator = Discriminator(module_name, hidden_dim, num_layers)
            # Define optimizers for different network components
            self.optimizer_e = torch.optim.Adam(
                chain(self.embedder.parameters(), self.recovery.parameters()),
                lr=learning_rate,
            )
            self.optimizer_g = torch.optim.Adam(
                chain(self.generator.parameters(), self.supervisor.parameters()),
                lr=learning_rate,
            )
            self.optimizer_d = torch.optim.Adam(
    ...
    def fit(self, data_train: np.ndarray, **kwargs: np.ndarray):
        ...
        conditions = (
            np.concatenate([c for c in kwargs.values()], axis=-1) if kwargs else None
        )
        conditions = (
        ...
        dataset = (
            TensorDataset(data_train, conditions)
            if kwargs
            else TensorDataset(data_train)
        )
        ...
        for epoch, frame in zip(range(self.epochs), cycle(r"-\|/-\|/")):
            ...
            for batch in batches_train:
                ...
                x_tilde = self.recovery(
                    h, conds
                )  # Recover the input from the embedded data
            ...
            if (epoch + 1) % (0.1 * self.epochs) == 0:
                print(
                    f"\rEpoch {epoch + 1} of {self.epochs} | loss_e {np.mean(loss_e):12.9f}"
                )
            else:
        ...
        for epoch, frame in zip(range(self.epochs), cycle(r"-\|/-\|/")):
            ...
            for batch in batches_train:
                ...
                h = self.embedder(x, conds)  # Embed the input data
                h_hat_supervise = self.supervisor(
                    h, conds
                )  # Supervise the embedded data
                g_loss = generator_loss_supervised(
                    h, h_hat_supervise
        ...
        for epoch, frame in zip(range(self.epochs), cycle(r"-\|/-\|/")):
            ...
            for _ in range(2):
                data_z = torch.rand(
                    data_train.shape, dtype=torch.float32, device=device
                )

                dataset = (
                    TensorDataset(data_train, data_z, conditions)
                    if kwargs
                    else TensorDataset(data_train, data_z)
                )
                ...
                for batch in batches_train:
                    ...
                    g_loss = generator_loss(
                        y_fake, y_fake_e, h, h_hat_supervise, x, x_hat
                    )
                    
                    # ADDED: Conditioning loss to enforce per-timestep behavior
                    if c is not None:
                        cond_loss = conditioning_loss(x_hat, c)
                        g_loss = g_loss + 50 * cond_loss  # Weight for conditioning
            ...
            self.losses.append(
                [epoch + 1, np.mean(loss_g), np.mean(loss_e), np.mean(loss_d)]
            )
            if (epoch + 1) % (0.1 * self.epochs) == 0:
                print(
                    f"\rEpoch {epoch + 1} of {self.epochs} | loss_g {np.mean(loss_g):12.9f} | "
    ...
    def transform(self, data_shape, **kwargs):
        ...
        data_z = torch.rand(
            size=data_shape, dtype=torch.float32, device=device, requires_grad=False
        )
        ...
        conditions = (
            torch.tensor(
                conditions, dtype=torch.float32, device=device, requires_grad=False
            )
            if kwargs
            else None
        )
        ...
        with torch.no_grad():
            for batch in batches:
                ...
                # FIXED: Don't squeeze - preserve 3D shape (batch, seq, features)
