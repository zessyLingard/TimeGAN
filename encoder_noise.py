"""
CTC-GAN Encoder (PCAP-style Output - Final)
"""
import os
import argparse
import subprocess
...
class NoiseGenerator(nn.Module):
    def __init__(self, module_name, z_dim, hidden_dim, num_layers, noise_scale=0.10):
        ...
        self.rnn = rnn_class(input_size=z_dim, hidden_size=hidden_dim,
...
# CONFIG
# ============================================================
MODEL_PATH = "helper/models/covert_channel_full_model.pth"
...
def bch_encode_file(input_file):
    ...
    if result.returncode != 0:
        raise RuntimeError("BCH Encode failed")
...
def main():
    ...
    parser.add_argument("--output", default="covert_ipd.txt")
    args = parser.parse_args()

    device = torch.device("cpu")

    print("[1] Load model & real data...")
    model = NoiseGenerator("gru", 1, 24, 3).to(device)
    model.load_state_dict(torch.load(MODEL_PATH, map_location=device)['generator'])
    model.eval()

    real_ipds = pd.to_numeric(
        pd.read_csv(REAL_DATA_PATH, header=None).iloc[:, 0],
        errors='coerce'
    ).dropna().values
    ...
    prob_to_time = interp1d(
        emp_cdf[unique_idx],
        sorted_real[unique_idx],
        bounds_error=False,
        fill_value=(sorted_real[0], sorted_real[-1])
    )

    print("[2] BCH encoding...")
    encoded_bits = bch_encode_file(args.file)

    print("[3] Generate GAN blueprint...")
    ...
    # ============================================================
    # BUILD IPD (PCAP STYLE)
    # ============================================================
    ipds = []
    bit_idx = 0

    sync_rng = np.random.RandomState(SHARED_SEED)
    ...
    for ai_time in gan_times_np:
        ...
        if ai_time < PAYLOAD_THRESHOLD:
            ipds.append(float(ai_time))
        else:
            current_bit = encoded_bits[bit_idx]

            # stealth hơn (giảm detect)
            dynamic_shift = sync_rng.uniform(0.005, 0.02)

            if current_bit == 0:
                target_time = max(ai_time - dynamic_shift, 0.00001)
            else:
                target_time = ai_time + dynamic_shift

            # jitter giống pcap (microsecond)
            smear = np.random.uniform(-0.00002, 0.00002)

            ipds.append(float(target_time + smear))
            bit_idx += 1

    # ============================================================
    # SAVE (PCAP-LIKE FORMAT)
    # ============================================================
    with open(args.output, 'w') as f:
        f.write(','.join(repr(v) for v in ipds))

    print(f"✅ Done! Generated {len(ipds)} IPDs -> {args.output}")
